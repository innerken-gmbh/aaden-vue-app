<template>
  <div style="height: 100vh;overflow-y: scroll;background: #f0f0f0" v-dragscroll>
          ...
          <template v-for="key in NeededKeys">
            <v-list-item dense :key="'config'+key">
              ...
              <template v-if="typeof Config[key]==='boolean'">
                <v-list-item-action>
                  <v-switch dense v-model="Config[key]" color="primary"></v-switch>
                </v-list-item-action>
              </template>
              <template v-else>
                <v-list-item-action>
                  <v-text-field dense hide-details outlined v-model="Config[key]" color="primary"></v-text-field>
                </v-list-item-action>
              </template>
            </v-list-item>
          </template>
